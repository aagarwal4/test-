...
File = namedtuple('File', 'path name root extension')
    ...
    for root, dir_names, file_names in os.walk(path):
        for f in file_names:
            ...
            if len(split_f) == 1:
                ...
                ext = split_f[-1]
                name = '.'.join(split_f[:-1])
            yield File(os.path.join(root, f), name, root, ext)
        
get_imgs = lambda path: [f for f in get_files(path) if f.extension.lower() in ['jpg', 'jpeg', 'png']]
...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        if subset == "queries":
            for img in get_imgs(dataset_dir):
